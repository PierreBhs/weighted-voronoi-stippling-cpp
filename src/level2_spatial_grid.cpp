...
auto run_level2(const config& cfg, const image_data& image, const execution_options& exec_options)
    -> result<level_summary>
{
    ...
    for (auto iter = 0uz; iter < cfg.max_iterations; ++iter) {
        ...
        assign_voronoi_grid(grid, *generators, voronoi, image.width, image.height);
    ...
    if (exec_options.allow_presentation) {
        ...
        if (cfg.display) {
            display_result(*generators, image.width, image.height, "Level 2: Spatial Grid Voronoi");
        }
    }
...
#if defined(STIPPLING_BUILD_LEVEL2_MAIN)
int main(int argc, char** argv)
{
    return stippling::run_cli_level(argc, argv, stippling::run_level2);
}
#endif
