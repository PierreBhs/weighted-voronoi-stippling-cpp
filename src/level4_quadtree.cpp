...
auto run_level4(const config& cfg, const image_data& image, const execution_options& exec_options)
    -> result<level_summary>
{
    ...
    if (exec_options.allow_presentation) {
        ...
        if (cfg.display) {
            display_result(*generators, image.width, image.height, "Level 4: Quadtree Voronoi");
        }
    }
...
#if defined(STIPPLING_BUILD_LEVEL4_MAIN)
int main(int argc, char** argv)
{
    return stippling::run_cli_level(argc, argv, stippling::run_level4);
}
#endif
